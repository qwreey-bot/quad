...
**[2026-09-01 추가, 같은 날 사용자가 기준 명확화] fable 구간 임시 강하 —
배치가 크고 난이도가 낮은 몰아 처리는 opus 서브에이전트로 내려 돌리고
결과 보고만 받는다.** 이건 "서브에이전트에게 맡길 종류의 일" 분류가
아니라 **작업의 부분 부분에서 잠시 모델을 내리는 것**을 서브에이전트로
구현하는 것이다 — 사용자 원문: *"'서브에이전트' 보단 부분 부분에서 잠시
opus 로 내리는걸 서브에이전트를 통해 하는거지."* 근거 둘(사용자):
시간(*"큰 모델은 tok/s 가 낮아"*)과 금액(*"심층 추론이 필요 없는 부분에선
opus 로 내려도 좋아"*). **판정 기준은 "작업 batch 가 커야" + 난이도가
낮을 것** — 예: Roblox MCP를 여러 번 호출하는 실측 반복, 스코프 조사
같은 대량 기계 작업(*"큰데 난이도는 낮거나 그런걸 몰아 처리하는거지.
탐사나 audit 와는 다른 느낌"*). 작은 단발 작업은 위 haiku/sonnet 기준
그대로이고, 탐사자·감사자는 각자의 규약(감사자 sonnet 고정 — 아래 "작업
방식")을 따른다. 설계·정합성 판단이 드는 일은 메인(fable)이 직접 — 메모리
`feedback-model-choice-for-reflection`의 "반영·설계 대화엔 모델을 낮추지
말 것"과 짝.
...
- **⭐ [2026-09-06 신설 — 사용자 지적] 백그라운드 에이전트의 "끝났는가"는 하네스
  ...
  적은 채 `status: completed` 알림을 보냈고, 메인 세션은 그 산문을 믿고 여러
  턴을 기다렸다 — 후속은 오지 않았다(사용자: *"자꾸 에이전트 끝났는데, 더이상
  회신 안 오는 상태가 되었는데 안 끝났다고 판단하게 되네"*). 규칙:
  1. **`task-notification`의 `status: completed`가 곧 종료다.** 그 알림은 "그
     에이전트가 살아 있는 자식 없이 멈췄을 때" 오므로, 본문이 뭐라 하든 그
     시점에 검증자·서브에이전트는 없다. 산문의 "대기 중"/"나중에 보내겠다"는
     약속이 아니라 미완의 보고다.
  2. **의심되면 `ListAgents`로 직접 본다** — 이 세션의 살아 있는 서브에이전트
     목록이 나온다(없으면 끝난 것). 알림을 기다리며 턴을 비우지 말 것.
  3. **미완 항목은 메인이 닫는다** — 근거가 이미 손에 있으면 그 자리에서
     판정(세 갈래)하고 원장에 "검증자 미완 — 메인 판정"으로 적는다(`H-339`가
     선례). 다시 띄우지 않는다(흐름당 리뷰 1회 규약).
  4. 리뷰·탐사 지시문엔 처음부터 **"마지막 메시지 하나에 최종 목록 전부, 미완
     항목은 '미완'으로 표시"**를 요구할 것 — 후속 송신을 약속하게 두면 3번
     상황이 반복된다.
- **⭐ [2026-08-18 실측] `/code-review`는 감사자를 대체하지 않는다 — 큰
  ...
  **diff 자체의 결함**(이번에 새로 쓴 서술 안의 모순, 새 API가 기존 계약과
  ...
  - **⚠️ [2026-08-26 사실 정정] *"`/code-review`는 사용자만 호출할 수 있다"*는
    ...
    - **⭐⭐ [2026-09-01 확정 — 사용자 합의] quad에서 문서 추론형 작업은
      쪼개지 않는다, 그리고 감사·리뷰는 진행을 막지 않는 유한 절차다.**
      사용자 진단: *"quad 는 여러 트레이싱을 나눠서 하면 안 될것 같은데
      항상 그러더라. 맥락이 확실히 분리 가능하지 않다고 생각"* — 이 레포
      리뷰의 실질은 diff가 아니라 **교차 문서 불변식 + 결정 이력**이라,
      조각 맥락 리뷰어는 (1) 이력을 몰라 새 메커니즘을 발명하거나(`H-141`
      `token` 사건 — 위 2026-08-27 규약의 발단), (2) 기추적 항목을 서로
      재발견한다(round12 리뷰 7건 중 3건). 같은 결론의 선례가 둘: 핸드오버
      워크플로 폐기(에이전트 67개 — 파일별 조각이 부정확 서술을 생산),
      감사자 "한 턴 하나 직렬" 규약. 따라서:
      1. **설계·정합성 검토의 정본 경로는 단일 맥락 직렬**(감사 루프 각도
         교대 + 메인 세션 손 트레이싱).
      2. **`/code-review`의 기대값은 "diff 국소 결함 + 기추적 교차 검증"
         까지다** — 깊은 설계 검토는 구조적으로 못 한다(맥락 없음). "high
         한 번 더 돌리면 깊은 게 나온다"는 기대로 재실행하지 말 것.
      3. **팬아웃은 근거가 코드 실행인 조각에만**(탐사자 스파이크, 단일
         주장 반증) — 문서 추론 조각은 자기 검증력이 없다.
      4. **감사·리뷰가 다음 작업을 막으면 안 된다**(사용자: *"감사로 인해
         다음 작업을 진행하지 못하게 되어버릴 수도 있거든"*) — 단위 끝
         절차는 상한 있는 유한 절차로: 감사 루프는 각도가 소진되면 잔여를
         보고하고 닫고, 리뷰는 흐름당 1회, 발견 반영의 재검증은 다음
         라운드 감사자 몫이지 새 리뷰가 아니다.
      계기가 된 관측: 2026-09-01 `/code-review high` 1회가 앵글 에이전트
      8개(A/B/C·Reuse·Simplification·Efficiency·Altitude·Conventions)를
      전부 fable로 굴렸고, 세션 누계가 캐시 읽기 404M/$589에 달했다 —
      조각 에이전트마다 같은 코퍼스·diff를 다시 싣는 구조가 비용의 주범.
