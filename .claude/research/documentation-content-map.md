...
20. **[2026-08-26 신설, 8라운드 `H-116`] quad 값은 만든 모듈 사본 안에서만
    ...
    **[2026-09-04 추가, M7 단위 ④]** `Modifier.Define`의 클래스 레지스트리
    (`defined`/`constructors`, `Modifier.luau` 모듈 수준)도 같은 목록에 든다 —
    한 사본에서 `Define`한 클래스는 다른 사본의 `As<Class>()`가 "unknown
...
22. **[2026-08-31 신설, `H-186` 확정] 교차 인스턴스 값 혼용은 UB** — 위
    20번(quad **두 벌** 공존)의 이웃: 같은 벌 안에서도 `New()`로 만든 서로
    다른 인스턴스의 값을 섞으면(`A.Effect(fn, B.Source(0))`, `A`의 핸들을
    `B.bindLifetime`에) 동작이 정의되지 않는다 — dep 쪽은 상대 백엔드의
    스텁/게이트가 엉뚱한 인스턴스를 지목하는 에러로, 생명주기 쪽은 조용히
    영영 안 울리는 것으로 나타난다(실측). 막지 않는 이유와 재검토 시점은
    `base/architecture.md` 13번의 `H-186` 항목이 소스
23. **[2026-08-31 신설, `H-170` 재확인] `ref:Wait`의 에러 전파 한계** —
