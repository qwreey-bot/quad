...
| `H-313` 확인 | **타이핑 계약 변화 확인 요청**(리뷰 지적 — ①로 처리했으나 사용자 가시 변화): children 자리는 마커만 보므로 `Frame { textButtonMod }`가 타입을 통과한다(클래스 소속 검사는 setter 호출 자리만). Luau 한계(8.8절)라 대안은 없고 수용 여부만 | 수용 |
