...
| `H10-12` | ② | 🟡 | **[같은 단위, strict 실측]** 특수 키가 `<Class>Param<E>`에 **타입으로 들어갈 자리가 없다** — `[number]: E` 인덱서라 `{ [q.OnChange "Text"] = fn }`이 strict에서 `Expected 'number', got 'OnChangeKey'`(+ 값도 `E` 유니언에 맞춰야 함). `AttributeKey`/`Tag` 키도 같은 사각(nocheck spec만 있어 드러나지 않았음). 런타임 무영향 | ⏳→ **OnChange 몫은 ✅**(`H10-14` 역전 — 배열부 값이라 생성기의 `E` 유니언 확장으로 흡수, 검증·추론까지 성립) / **`AttributeKey` 몫만 잔여**: `[AttributeKey "x"] = v`는 값 세팅이라 해시부가 자연스럽지만 `<Class>Param<E>`에 타입 자리가 없는 사각은 그대로(`Tag`는 배열부 값이라 인덱서 문제가 아니라 `E` 확장 누락 — 정정). 갈래 (a) 현상 유지 + 문서화(`::` 캐스트) / (b) 인덱서를 `[number \| AttributeKeyObject]: E \| any`로 넓힘(원소 타입 정밀도 손실) / (c) 표면 변경. **사용자 판단**, 급하지 않음 |
