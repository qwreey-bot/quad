...
| `H-275` | ② | 4 | 🟡 | (탐사자 F-4) **형상이 바뀌는 배열 재`drive`의 지원 여부가 미명시** — 축소(`{o1,o2,o3}`→`{o1,o2}`)는 자리 3을 아무도 철거 안 해 조용한 잔존(o3 발화 지속, `bk.N=3`), 교환(`{oA,oB}`→`{oB,oA}`)은 (A) 분기가 아직 자리 2에 묶인 oB를 bind하다 already-bound 크래시 + slot 1 `H-103` NOOP 잔존. 같은 형상 재drive는 sanctioned(spec.leaf 3), 재진입 drive는 `H-241`로 별도 — 이건 **순차 재호출** 축 | ✅ [2026-09-01 (a) 확정 + 범위 확장 — "형상 불문 drive 1회"] `dispatch-core-plan.md` 두 UB 문단, §4 상세 행의 ✅가 소스 |
