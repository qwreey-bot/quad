...
| **Luau 타입체커** (`luau-analyze` 또는 `luau-lsp`) | 같은 릴리즈에 포함된 `luau-analyze`, 또는 `luau-lsp analyze`/에디터 인라인 진단 | 06(타입 부분), 08, 09, 12, 13, 14, 15, 16, 21, 23, 26, 28, 29, 30, 31, 32, 33 |
...
| `11-modifier-illegal-value-error.luau` | **[2026-09-06 폐기 → `done/`]** M7 `spec.modifier`가 대체(`STATUS.md` `done/` 표). Modifier 필드에 Ref/PreRef/Observer/Effect/Slot/Modifier가 들어오면 즉시 error, State/Source가 확정하는 값이 Modifier면 즉시 error(2026-08-09 세션에 "UB"에서 전환된 규칙) | `modifier-plan.md` "Modifier 필드에 핸들러 계층 값(Ref/PreRef/PostRef/Observer/Effect/Slot/Modifier)이 들어오면 즉시 error" 절 + 7번 절 **⚠️ [2026-08-26 이동 → `rewrite-required/`, 8라운드 `H-122`/`H-123`]** 검증 코드의 Store 생성자가 **eager `Source(v)`** 모델을 박제하고 있다 — 명시적 초기화 확정(2026-08-25) 이후 **`defaults` 경로에선** Store가 `Source`를 만들지 않는다(동적 키 창구 `store:Of(name)`은 여전히 만든다 — 그래서 가드가 `Source` 생성자로 갔다). 재작성 지침은 `STATUS.md`의 `rewrite-required/` 표. |
...
| `21-type-store-undeclared-key-rejected.luau` (타입체크 전용) | **[2026-08-19 신규]** `Store<{field: T}>`로 선언 안 된 이름에 dot-access하면 `type function`이 합성한 결과 타입(`ProcessStoreType`, `16`과 동일)에 그 프로퍼티가 없어 타입 시간에 거부되는지 — `store-plan.md`가 "아마 그럴 것"으로만 적어뒀던 걸 M0에서 실측. 통과: 미선언 키 접근 2건이 정확히 `TypeError`로 걸림 | `store-plan.md` "Store = Source들의 이름 붙은 모음" 절의 "[확인 요구, 2026-08-18 구현 전 QA]" 항목, `todos.md` 00번  **⚠️ [2026-08-25 이동, `rewrite-required/`]** `16`의 `ProcessStoreType`을 재사용하므로 같이 낡았다. **검증 대상(미선언 키는 타입 에러)은 그대로 유효**하고 새 모양에서도 성립함이 확인됐다(`store.nope` 거부) — 새 `Store<T>` 선언으로 바꿔 쓰기만 하면 된다 |
| `22-runtime-ref-preref-postref-brand.luau` | **[2026-08-19 신규]** 구 `13`의 런타임(B) 절반을 분리한 것 — `isPreRef`/`isPostRef`가 같은 층위의 배타적 형제(둘 다 `isRef`엔 `true`, 서로에겐 `false`)인지, Leaf 핸들러 흉내(`isRef(v) and not isPreRef(v) and not isPostRef(v)`)가 Ref/PreRef/PostRef 셋을 정확히 갈라내는지. **[2026-08-21] `rewrite-required/`로 이동** — 파일이 직접 구현해 쓰는 `Brand.set`/`Brand.get`이 인스턴스 브랜드 재작성으로 역전된 옛 API가 됐다(검증 대상 자체는 그대로 유효, 상태와 재작성 지침은 `STATUS.md`가 소스). **[2026-09-06 폐기 → `done/`]** M8 `spec.preref` 1절이 대체(`STATUS.md` `done/` 표) | `ref-plan.md`의 "`PostRef`" 절, `brand-plan.md`의 "⭐ 구현 — 인스턴스 브랜드" 절 |
...
| `30-type-onchange-array-descriptor.luau` (타입체크 전용) | **[2026-09-03 신설, `OnChange` 배열부 역전 착수 전]** 이름을 인자로 받는 팩토리 `OnChange(name, fn)`의 타이핑 경계 — 제네릭 `K`는 리터럴에서 싱글톤이 안 남고, 큰 싱글톤 유니언 `K &`·오버로드 교집합은 too complex; **`K & keyof<PropTypes>` + `index<PropTypes, K>`만 통과**하며 오타·콜백 타입·클래스 밖 이름을 잡고 무주석 파라미터까지 추론. `State<T>` 불변성 캐비엇(캐스트 경유). 기대 음성 NEG 4줄. 실물 규모는 `quad-roblox/test/spec.onchangetypes.luau` | `onchange-plan.md` "타이핑" 절, `typing-limits.md` 8.7절 |
| `31-type-union-subtyping-recursive-collision.luau` (타입체크 전용) | **[2026-09-04 신설, M7 단위 ④ 착수 전]** 새 솔버 유니언 서브타이핑 결함의 최소 재현(자기 타입 참조 함수 필드 + 유니언 멤버의 같은 이름 함수 필드 → 마커 불일치를 무시하고 통과)과 그 처방 위에서 성립하는 클래스 태그 마커·검사형 `As<Desc>()`·무검사 `As<<T>>()`·`Into<Class>` 인터페이스의 소형 타이핑, 그리고 교집합 Param `{ [number]: E } & Props`가 리터럴에서 성립하지 않는 증거. 기대 진단은 NEG 11줄뿐; **결함 재현 줄에 진단이 생기면 결함이 고쳐진 것**이라 `Apply` any 처방을 되돌릴 수 있다 | `modifier-plan.md` 11절, `typing-limits.md` 8.9절, `audit/m7-unit4-as-modifier-2026-09-04.md` |
| `33-type-tween-ctor-intersection-vs-setmetatable.luau` (타입체크 전용) | **[2026-09-06 신설, M11 단위 ①]** 제네릭 `__call` 콜러블 네임스페이스(`Tween{...}`)의 타입 선언 — 8.6절 규칙(`setmetatable<A, B>`)은 `T`가 `*error-type*`으로 오염돼 음성 셋을 전부 놓치고, 함수∩테이블 교집합은 셋 다 잡는다(`H-324`, 8.6절 조건부 예외의 근거) | `typing-limits.md` 8.6절, `tween-plan.md` "최종 타입" 절 |
| `32-type-ref-children-contravariant-marker.luau` (타입체크 전용) | **[2026-09-04 신설, M8 단위 ③ 착수 전]** children 유니언에서 `Ref<T>`의 클래스 소속을 어떻게 검사하나 — `Ref<Frame?>`를 직접 넣는 형은 실물 규모에서 8.9절 결함으로 새고(소형은 잡힘 — 헤더 참고), `Ref<T>`에 반공변 팬텀 필드 `read __quadRefAccepts: (T) -> ()`를 두고 유니언엔 `{ read __quadRefAccepts: (<Class>) -> () }`만 넣으면 상위 클래스 박스는 받고 형제·`Ref<nil>`은 거부. NEG 3줄 | `ref-plan.md`, round18 `H-321`, `typing-limits.md` 8.9절 |
