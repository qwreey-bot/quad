...
의존** — `quad-base`는 런타임 주입 패턴(**[2026-09-02 `H-305` (d′)]**
현행 표면은 `quad:UseProvider(QuadRoblox)` — 옛 `QuadRoblox(Quad)` 직접
호출형에서 교체, `module-lifecycle-plan.md`)으로 **런타임
...
│       ├── Modifier.luau         # flatten-before-dispatch, immutable 체이닝, 제네릭 `__index` 필드 setter 합성 + `:Apply`/`:Peek`/`Overridden`(`base/modifier-plan.md`) — **[2026-09-04 M7 단위 ① 구현]** 값 런타임 전부(콜러블 `Modifier()`, 예약 메소드 우선 `__index`, 내부 저장소 `FieldsKey` 이중 clone·frozen, 4분기 setter, 핸들러 계층 값 error, round17 `H-309`); **[단위 ② 구현]** `flatten`·`ProcessedModifier` export도 이 파일(`H-311`) — 호출은 `Dispatch/init.luau`의 `drive` 첫 pre-pass
│       ├── Tag.luau              # **[2026-09-03 `H10-1` 재편 — M10 편입]** 값 타입+immutable clone 체이닝(`Tag(...)`/`:Added`/`:Removed`/`:Contains`/`:Apply`/`Merged`/`:Names`) **+ 참조 카운트 `TagHandler`+FALLBACK 자기 등록까지 이 한 파일**(`H-278` 형 — 옛 `Dispatch/Tag.luau`+`TagFallback.luau` 분할 폐기, "FallbackHandler"는 등록 엔티티 이름으로만). 엔진 호출은 주입된 addTag/removeTag(`base/tag-plan.md`)
│       ├── Attribute.luau        # **[`H10-1` 재편]** 그룹 값 타입+API(`Attribute(store1, store2, ...)`/`Merged`/`Overridden`/`:NameMap`, `Tag`와 동형) **+ `AttributeGroupHandler`+FALLBACK 자기 등록까지 이 한 파일** + **[2026-09-03]** 타입드 스칼라 슈가 `StringAttribute(name, value)`/`Number…`/`Boolean…`(= 단일 항목 그룹, raw 값 타입 검사·nil 거부) (`base/attribute-plan.md`)
│       ├── AttributeKey.luau     # **[`H10-1` 재편]** 단일 키 `AttributeKey(name)`(**[2026-09-03]** 무타입 프리미티브 — 제네릭 폐기, 스칼라 패밀리는 `Attribute.luau`의 배열부 슈가로 이동) + 이름별 weak 캐시 **+ `AttributeKeyHandler`(이름 claim)+FALLBACK 자기 등록까지 이 한 파일** — 엔진 고유 타입 패밀리(Color3Attribute류)만 백엔드 소속(`base/attribute-plan.md` "패키지 배치" 절)
...
│       │   ├── (Tag/AttributeKey/Attribute 없음) # **[2026-09-03 `H10-1` 재편]** 이 세 핸들러 파일 분할은 `H-278` 이전 서술 — 실물은 위 top-level `Tag.luau`/`AttributeKey.luau`/`Attribute.luau`가 값+핸들러+FALLBACK 자기 등록을 전부 가진다(알고리즘 서술은 각 base 문서 그대로 유효)
...
└── quad-roblox/
    ...
    └── src/
        ...
        ├── Handlers/
        │   ├── Property.luau      # 일반 프로퍼티 세팅 + `isTween(realv)` 분기(3-상태 릴레이션 슬롯 `{Tween, Value}|true|nil` — **[2026-08-28 `H-155`]** 옛 표기 `RobloxTween|true|nil`은 `Tween.Finish`가 목표값을 알아야 해 테이블로 정정, `tween-plan.md` "3-상태 저장" 절이 소스 —, hasBeenSet 억제, override 정책) — 구 `Handlers/Tween.luau`(높은 우선순위 store-bind 핸들러)는 폐기(`archive/tween-special-bind-key-reversed.md`)
        │   ├── Event.luau         # ReflectionService 기반 자동 판별 — **[2026-09-03 M10 구현]** `GetEventsOfClass` 클래스별 캐시(상속 포함, Property 집합과 분리 — 실측), 키 매치라 `v == nil`도 잡아 그때는 Connect 없이 `Void`, retractor가 Connection을 캡처해 Disconnect(`base/event-plan.md`)
        │   ├── InstanceShorthand.luau # **[2026-09-06 M10 잔여 구현, round20]** `UICorner`/`UIPadding`/`UIPaddingOffset`/`UIScale` 특수 키 — 룩업 표 하나, 우선순위 `NORMAL + 1`(`H-138`), 관리 자식은 `D.New` 경로로 만들고 `Relate`로 조회, 자식 프로퍼티는 `Dispatch.process(child, prop, v, 1)` 위임(Tween은 `:Mapped`), `nil`이면 `retractFrom` 뒤 파괴(`base/ui-shorthand-plan.md`)
        │   ├── OnChange.luau      # `OnChange(name, fn)` **배열부 디스크립터** 팩토리+Handler(**[2026-09-03 역전]** 옛 해시부 특수 키 `[OnChange "x"] = fn`은 `archive/onchange-hash-key-reversed.md`), `GetPropertyChangedSignal` 바인딩, 길이 0 말단(Tag 관용구), 초기값 발화 계약(배열→해시 순서), 디스크립터 브랜드는 모듈 로컬 weak-key 집합(quad-roblox는 quad-base를 require하지 않음), `OnChange`는 `RobloxExtension = { D, OnChange }`로 `UseProvider` 병합에 실리고 타입은 생성 `OnChangeFn`(`D/init.luau` — `K & keyof<PropTypes>`/`index<PropTypes, K>`, `base/onchange-plan.md`)
