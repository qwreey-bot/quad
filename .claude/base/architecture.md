...
│       ├── Modifier.luau         # flatten-before-dispatch, immutable 체이닝, 제네릭 `__index` 필드 setter 합성 + `:Apply`/`:Peek`/`Overridden`(`base/modifier-plan.md`) — **[2026-09-04 M7 단위 ① 구현]** 값 런타임 전부(콜러블 `Modifier()`, 예약 메소드 우선 `__index`, 내부 저장소 `FieldsKey` 이중 clone·frozen, 4분기 setter, 핸들러 계층 값 error, round17 `H-309`); flatten·`ProcessedModifier`는 단위 ②
