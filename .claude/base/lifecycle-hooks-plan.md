...
- **불리는 시점**: 이 인스턴스의 children(과 그 서브트리 전체)과
  프로퍼티/이벤트가 **전부 세팅된 뒤**.
- **⚠️ 보장하지 않는 것**: 이 인스턴스가 부모에 붙었는지 여부 — **어느
  쪽도 보장 안 함**(**[2026-09-04 정정, M8 brief §0 Q6 — 사용자]** 옛 문장
  "부모에 붙은 뒤가 아님"은 리터럴 중첩 경로에만 맞고, `Claim`이나 미리
  세팅된 `.Parent`면 붙은 채로 fire된다 — `ref-plan.md` "`PostRef`" 절).
  `PostRef`는 자기 아래(서브트리)의 완성만 보장하고 자기 위(조상 체인)는
  없을 수도 있음 — React `componentDidMount`가 DOM 삽입 **후**인 것과
...
- **`OnRendered`/`PostRef` — 이름 확정(2026-08-14 아홉 번째 세션).**
  ...
  원래 우려는 유효하고, 여기에 **"이 인스턴스가 부모에 붙었는지는
  보장하지 않는다(기본 경로에선 아직 안 붙어 있음)"**는 캐비엇까지 겹침
  (**[2026-09-04 정정]** 옛 "붙기 전에 불린다"는 M8 brief Q6로 무보장이 됨). 즉 이 이름은 **"React에서 오는 사람이
